...
pub fn run() -> Result<()> {
    ...
    Application::new()
        ...
        .run(move |cx| {
            ...
            // Control flag for the optional animation task
            let mut anim_running: Option<StdArc<AtomicBool>> = None;
                ...
                while let Ok(event) = event_rx.recv_async().await {
                    match event {
                        DaemonEvent::Window(WindowEvent::RequestHide) if visible => {
                            ...
                            if let Some(flag) = anim_running.take() {
                            ...
                            let result = if !visible {
                                cx.update(|cx| {
                                    match window::create_and_show_window(
                                        ...
                                        Ok(lw) => {
                                            // Clone the view entity for the animation task
                                            let view_entity = lw.launcher_view.clone();
                                            launcher_window = Some(lw);
                                            visible = true;

                                            // Start animation ticker (runs while `anim_running` is true)
                                            let running = StdArc::new(AtomicBool::new(true));
                                            anim_running = Some(running.clone());
                                            let _ = cx.spawn(async move |cx: &mut gpui::AsyncApp| {
                                                ...
                                                loop {
                                                    ...
                                                    let _ = cx.update(|cx| {
                                                        view_entity.update(cx, |_launcher, cx| {
                                                            cx.notify();
                                                        });
                                                    });
                                                }
