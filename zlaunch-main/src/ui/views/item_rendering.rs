...
fn render_application(
    ...
    let mut item = item_container(row, selected)
        .child(render_icon(app.icon_path.as_ref(), selected))
...
fn render_window(win: &crate::items::WindowItem, selected: bool, row: usize) -> Stateful<Div> {
    let mut item = item_container(row, selected)
        .child(render_icon(win.icon_path.as_ref(), selected))
        .child(render_text_content(
            &win.title,
...
fn render_action(act: &crate::items::ActionItem, selected: bool, row: usize) -> Stateful<Div> {
    ...
    let mut item = item_container(row, selected)
        .child(render_phosphor_icon(icon, selected))
        .child(render_text_content(
            &act.name,
...
fn render_search(search: &crate::items::SearchItem, selected: bool, row: usize) -> Stateful<Div> {
    let mut item = item_container(row, selected)
        .child(render_phosphor_icon(Some(search.icon()), selected))
...
fn render_ai(ai: &crate::items::AiItem, selected: bool, row: usize) -> Stateful<Div> {
    let mut item = item_container(row, selected)
        .child(render_phosphor_icon(Some(ai.icon()), selected))
...
pub fn item_container(row: usize, selected: bool) -> Stateful<Div> {
    ...
    // Add a subtle border for selected items to make selection more distinct
    if selected {
        // Use neon accent color for selection outline so hover/selection are prominent
        container = container
            .border_1()
            .border_color(theme.neon_color.alpha(theme.neon_intensity.min(1.0)));
    }
...
pub fn render_icon(icon_path: Option<&PathBuf>, selected: bool) -> Div {
    let theme = theme();
    let mut size = theme.icon_size;
    if selected {
        size = size + px(6.0);
    }
...
pub fn render_phosphor_icon(icon: Option<PhosphorIcon>, selected: bool) -> Div {
    let theme = theme();
    let mut size = theme.icon_size;
    if selected {
        size = size + px(6.0);
