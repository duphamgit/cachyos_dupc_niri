...
pub struct LauncherView {
    ...
    /// Whether the mouse is hovering the launcher panel
    panel_hovered: bool,
    // (open animation removed)
...
impl gpui::Render for LauncherView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Compute neon pulse using theme settings (used for hover highlight)
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| d.as_secs_f32())
            .unwrap_or(0.0);

        let freq = theme.neon_frequency.max(0.01);
        let phase = now * std::f32::consts::TAU * freq;
        let pulse = 0.5 + 0.5 * (phase.sin());
        let eased = pulse * pulse * (3.0 - 2.0 * pulse);

        // Neon accent: ensure a visible base alpha so glow remains visible
